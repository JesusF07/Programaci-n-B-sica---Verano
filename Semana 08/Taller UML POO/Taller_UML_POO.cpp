...
public:
    ...
    void mostrar() const {
        if (entradas.empty()) {
            cout << "  Historia clínica vacia" << endl;
            return;
        }
...
private:
    ...
    void validarDatos() const {
        ...
        if (dni.length() != 8 || !regex_match(dni, regex("\\d{8}"))) {
            throw invalid_argument("DNI debe tener exactamente 8 digitos numericos");
        }
...
private:
    ...
    void validarDatos() const {
        ...
        if (codigo.empty()) {
            throw invalid_argument("El codigo del doctor no puede estar vacio");
        }
    }
...
public:
    Cita(int id, Paciente* p, Doctor* d, const string& fecha, const string& hora, const string& motivo)
        ...
        if (motivo.empty()) {
            throw invalid_argument("El motivo de la cita no puede estar vacio");
        }
    }
...
public:
    ...
    void agendarCita() {
        try {
            ...
            if (!paciente) {
                throw EntidadNoEncontrada("No se encontro paciente con DNI: " + dniPaciente);
            }
            
            // Buscar doctor
            cout << "Codigo del doctor: ";
            getline(cin, codDoctor);
            Doctor* doctor = buscarDoctor(codDoctor);
            if (!doctor) {
                throw EntidadNoEncontrada("No se encontro doctor con codigo: " + codDoctor);
            }
    ...
    void listarCitasPaciente() {
        try {
            ...
            if (!paciente) {
                throw EntidadNoEncontrada("No se encontro paciente con DNI: " + dni);
            }
    ...
    void listarCitasDoctor() {
        try {
            ...
            if (!doctor) {
                throw EntidadNoEncontrada("No se encontro doctor con codigo: " + codigo);
            }
...
int main() {
    ...
    cout << "======================================" << endl;
    cout << "   CLINICA SALUD+ - SISTEMA DE CITAS  " << endl;
    cout << "======================================" << endl;
    ...
    do {
        try {
            ...
            if (opcion < 1 || opcion > 8) {
                throw out_of_range("Opción no valida. Ingrese un numero entre 1 y 8.");
            }
